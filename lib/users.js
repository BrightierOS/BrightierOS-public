...
const ROLE_PERMISSIONS = {
  admin: ['users:manage', 'roles:view', 'settings:manage', 'logs:view', 'files:all', 'plugins:all', 'store:all', 'services:all', 'infrastructure:all'],
  editor: ['roles:view', 'logs:view', 'files:all', 'plugins:all', 'store:all', 'services:view', 'infrastructure:view'],
  viewer: ['roles:view', 'logs:view', 'files:read', 'services:view', 'infrastructure:view'],
};
...
function hasPermission(role, perm) {
  ...
  // Permissão hierárquica: "<grupo>:all" concede qualquer "<grupo>:<ação>".
  // Ex.: quem tem 'files:all' também pode 'files:read' / 'files:write'.
  const sep = perm.indexOf(':');
  if (sep > -1) {
    const group = perm.slice(0, sep);
    if (perms.includes(`${group}:all`)) return true;
  }
  return false;
}
