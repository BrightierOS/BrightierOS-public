...
function run(cmd, opts = {}) {
  return new Promise((resolve) => {
    exec(cmd, { windowsHide: true, maxBuffer: 8 * 1024 * 1024, ...opts }, (err, stdout, stderr) => {
...
async function listServices() {
  ...
  try {
    if (platform === 'linux') {
      ...
      // m[2]=LOAD (sempre "loaded"), m[3]=ACTIVE ("active"/"inactive"/"dead"),
      // m[4]=SUB+" "+DESCRIPTION. Usamos m[3] para o status principal.
      ...
      if (!err && stdout) {
        stdout.split('\n').forEach((line) => {
          ...
          services.push({
            ...
            status: m[3] === 'active' ? 'running' : 'stopped',
        ...
        const { stdout: svc } = await run('service --status-all 2>/dev/null || true');
        if (svc) {
          svc.split('\n').forEach((line) => {
