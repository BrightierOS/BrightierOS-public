...
async function listServices() {
  ...
  try {
    if (platform === 'linux') {
      ...
      stdout.split('\n').forEach((line) => {
        ...
        if (nm) { current = { id: nm[1].trim(), name: nm[1].trim(), type: 'service', managed: 'sc', category: 'system', canControl: true }; return; }
  ...
  } catch (_) { /* ambiente sem gerenciador de serviços — só BrightierOS + plugins */ }
  return services;
}
