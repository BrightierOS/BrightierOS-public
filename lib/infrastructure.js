...
const DATA_DIR = dataUtils.DATA_DIR;
const NODES_FILE = path.join(DATA_DIR, 'infrastructure.json');

const LOCAL_NODE_ID = 'local';

// Limita a quantidade de requisições paralelas para não sobrecarregar o sistema.
...
function readNodes() {
  dataUtils.ensureJsonFile(NODES_FILE, '[]');
  return dataUtils.readJsonFile(NODES_FILE, []);
}

function writeNodes(nodes) {
  dataUtils.writeJsonFile(NODES_FILE, nodes);
}
...
function listNodes() {
  ...
  return nodes.map((n) => (n.id === LOCAL_NODE_ID ? { ...localNode(), ...n, credentialsConfigured: false } : _sanitizeNode(n)));
}
...
// Usa limitador de concorrência para não sobrecarregar a rede/máquina.
async function checkAllNodes() {
  const nodes = listNodes();
  const remote = nodes.filter((n) => n.id !== LOCAL_NODE_ID);
  await parallel(remote, 5, (n) => checkNode(n.id).catch(() => null));
  return listNodes();
}
